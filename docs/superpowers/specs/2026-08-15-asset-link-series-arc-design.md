...
## 8. 上传时按筛选自动关联（补充，2026-08-15）

工作台上传时，前端根据**当前筛选**把关联一并提交（后端 `POST /api/assets/upload` 增加可选 link 参数）：

| 当前筛选 | 上传后自动关联 |
|----------|----------------|
| 关联类型=人物，且选中具体人物 | `linkType=CHARACTER` + 该 characterId |
| 关联类型=系列，且选中具体系列 | `linkType=SERIES` + 该 seriesId |
| 关联类型=篇章，且选中具体篇章 | `linkType=ARC` + 该 arcId |
| 关联类型=篇章，仅选系列未选篇章 | 不自动挂篇章（避免歧义） |
| 全部 / 未关联 / 未选具体对象 | `linkType=NONE`（无自动关联） |

人物页既有 `POST /api/characters/{id}/assets/upload` 行为保持：上传后挂该人物。
